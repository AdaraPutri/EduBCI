export const paragraphs = [
  ...
  },
  {
    paragraphId: 11,
    type: "neutral",
    text: `Saving a file on a computer means writing the current information to long-term storage so it does not disappear when the power is turned off. Text documents, pictures, and simple program data are all saved in this way. When you reopen the file later, the computer reads the stored information and shows it to you in the same form as before. Programs often combine reading and writing files with basic variables and arrays to remember user choices.`
  },
  {
    paragraphId: 12,
    type: "neutral",
    text: `Learning basic programming usually starts with a small set of ideas that build on each other: variables to store values, if statements to make simple choices, and loops to repeat actions. Arrays are introduced as a convenient way to keep many related values together. Functions then help group related steps so they can be reused easily. These concepts are enough to create simple tools, such as calculators, note-taking apps, or small games, without needing advanced mathematics or complex theory.`
  },
  ...
  },
  {
    paragraphId: 35,
    type: "fully_confusing",
    text: `In highly scalable databases, cutting-edge research on weakly consistent replicated data types introduces semantics like causal plus plus consistency, where operations may be observed in different orders at different replicas, yet still satisfy algebraic convergence guarantees that are difficult to reconcile with everyday intuitions about transactions. PhD projects often specify these behaviors using operational semantics over version vectors and happens-before relations, so proving that all replicas eventually agree requires constructing intricate invariants across asynchronous executions where updates, merges, and failures interleave in patterns that rarely appear in textbook examples. Even when systems expose high-level APIs that look like ordinary key-value stores, subtle differences in conflict-resolution policies can lead to lost updates or surprising merges that only manifest under concurrent workloads, making empirical debugging an unreliable way to understand correctness. As a result, application developers are sometimes advised to change their data models entirely, adopting patterns like event sourcing or CRDT-specific abstractions, which shifts complexity from the database layer into the application logic where it is even harder to reason about formally.`
  },
  {
    paragraphId: 36,
    type: "fully_confusing",
    text: `Microarchitectural side-channel research at the dissertation level shows how speculative execution, cache hierarchies, and branch predictors can leak secrets through tiny timing differences, so analyzing an attack often requires reasoning simultaneously about high-level code, compiled binaries, and undocumented hardware behaviors. Proof-of-concept exploits frequently chain multiple microarchitectural quirks, for example using a mistrained branch predictor to speculatively access protected data and then encoding the secret bits into cache eviction patterns, which are later reconstructed by precise measurement loops that look harmless in source form. Defenses, meanwhile, range from inserting fences and random delays to redesigning entire cache architectures, but evaluating their effectiveness rigorously requires constructing attacker models that capture not only current techniques but also plausible future variations that have not yet been demonstrated. For newcomers, it is therefore difficult to tell which published attacks reflect fundamental weaknesses and which depend on fragile laboratory setups, yet the literature often assumes familiarity with hardware timing, speculative pipelines, and low-level measurement techniques that are rarely taught explicitly.`
  }
];
